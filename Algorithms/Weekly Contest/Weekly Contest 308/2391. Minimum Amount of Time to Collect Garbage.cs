...
public class Solution 
{
    ...
    {
        int ans = 0;
        foreach (var c in "MPG")
        {
            ans += CountTime(c, garbage, travel);
        }
        return ans;
    }

    public int CountTime(char c, string[] garbage, int[] travel)
    {
        int ans = 0, ti = 0, n = garbage.Length;
        for (int i = 0; i < n; i++)
        {
            if (garbage[i].Contains(c))
            {
                ti = i;
                ans += garbage[i].Count(x => x.Equals(c));
            }
        }
        ans += travel.Take(ti).Sum();
        return ans;
    }
}